...
## 1. 概述
Wi-Fi 7，技术标准名称为 **IEEE 802.11be EHT (Extremely High Throughput)**，旨在提供相比 Wi-Fi 6/6E 更高的数据传输速率（理论峰值达 46 Gbps）、更低的延迟和更高的频谱效率。它主要面向 8K 视频流、AR/VR/XR、工业物联网等对带宽和延迟极其敏感的应用场景。
...
### 4.1 调制与与解调算法 (Modulation & Demodulation)
...
*   **动作**: 操作系统内核网络栈将数据包（MSDU）通过总线（PCIe/SDIO）传送给 Wi-Fi 芯片的 RAM (DMA操作)。
*   **数据状态**: **明文 (Plaintext)**。
...
*   **步骤 3: 附加认证数据构造 (AAD Construction)**
    *   **算法**: **AAD Construction**.
    *   **逻辑**: 提取 MAC 头部的关键字段（如源/目的MAC地址、Frame Control）组成 AAD。
